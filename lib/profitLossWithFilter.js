...
module.exports = function(models){
    ...
    async function ProfitLossData(data){
        ...
        let costAndQuan = await costAndQuantity(movies,data)
        let sellingData = await rentData(movies,data)
...
//finds cost and the quantity with respect to movie
    function costAndQuantity(movies,dateRange){
        var movieArray = []
        return new Promise ((resolve,reject) =>{
            movies.forEach((movie,index) => {
                ...
                return models.Purchases.findAll({
                    where: {
                        MovieId: movie.id,
                        createdAt: {$gte: dateRange.startDateRange, $lte:dateRange.endDateRange}
                    },
                    include:[
                        {model: models.Movies}
                    ]
    ...
    function rentData(movies,dateRange){
        var movieArray = []
        return new Promise ((resolve, reject) =>{
            movies.forEach((movie, index) =>{
                ...
                return models.Rental.findAll({
                    where: {
                        rentPaidStatus: 1,
                        MovieId: movie.id,
                        updatedAt: {$gte: dateRange.startDateRange, $lte: dateRange.endDateRange} 
                    }
                }).then((rentals) =>{
